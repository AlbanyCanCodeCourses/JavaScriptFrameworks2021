...
describe("<App />", () => {
  ...
  it("should hide the links when the hide button is clicked", () => {
    ...
    wrapper.find("#hide-links").simulate('click');
    expect(wrapper.find("Links")).to.have.lengthOf(0);
  });
  ...
  it("should add the user input to the new link <li> on the screen when the add button is clicked", () => {
    ...
    const addLink = wrapper.find('button').first();
    inputs.first().simulate('change', {target: {value: "Omfg Dogs"}});
    inputs
      .last()
      .simulate("change", {
        target: { value: "http://www.omfgdogs.com/" },
      });
