...
function App() {
  ...
  /**
   * When a user is logged in, a token is stored in state here
   */
  ...
  const login = async () => {
    ...
    try {
      const response = await axios("http://localhost:7000/api/login", {
        method: "POST",
        headers: {
  ...
  /**
   * When the user is logged out, the token is empty.
   */
  const logout = () => {
    setToken("");
  };
